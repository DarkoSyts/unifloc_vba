...
print("     \n")
minimize(calc_well_plin_pwf_atma_for_fsolve, [0.5], bounds = [[0,1]])
...
class well():
    ...
    def calc_well_plin_pwf_atma_for_fsolve(self, c_calibr_head_d):
        self.c_calibr_head_d = float(c_calibr_head_d)
        ...
        self.result = UniflocVBA.calc_well_plin_pwf_atma(self.qliq_m3day, self.watercut_perc, self.p_wf_atm,
                                                    ...
                                                    self.PVTstr, self.ESPstr, c_calibr_head_d=self.c_calibr_head_d)

        self.plin_calculated_atm = result[0][0]
        result_out = (self.plin_calculated_atm - self.p_wellhead_data_atm) ** 2
        return result_out

    def calc_well(self):
        fsolve(self.calc_well_plin_pwf_atma_for_fsolve, 0.5, xtol=1)
        minimize(self.calc_well_plin_pwf_atma_for_fsolve, [0.5], bounds=[[0,100]])
